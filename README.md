...
Handoff generation collapses an embedded prior continuation prompt to a short
pointer, preventing repeated account switches from causing nested handoffs to
grow without bound. The full conversation appendix is included by default and
can be disabled by callers of the pure `buildHandoff` function.
...
Handoff 生成会将之前嵌入的续接提示压缩为简短指针，避免多次切换账号后内容
不断嵌套增长。完整对话附录默认包含；调用纯函数 `buildHandoff` 时可以关闭。
